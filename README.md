...
>이 프로젝트는 기존 프로그램의 실행 환경에 대한 의존성을 줄이고,
> 현재 Python 환경에서 동일한 `.lst` 데이터를 활용할 수 있도록 하는 것을 목표로 한다.
...
│
├─ phase1-compatible/            # 1차: 윈도우 11 환경에서 가능하게끔 설정
│  ├─ makegraph.py
│  ├─ samples/
...
- **1차 (`phase1-compatible`)**: 실용성·호환성 우선. 
> Python에서 기존 `.lst`를 읽어 그래프를 생성하는 게 목표.
  
- **2차 (`phase2-reproduction`)**: 원본과의 동작 일치성 우선. 
>  같은 `.lst`를 원본 EXE와 재구현판에 각각 입력해 레이아웃/스타일/오류 처리를 비교한다.
...
최신 Windows 환경에서 이 레거시 실행 구조와의 호환성 문제가 발생할 수 있어 재구현이 필요하다.
> => 이런 문제도 있고 .ttl로 분석을 진행하며 아예 GUI 새 버전을 만들었다.
...
> 참고> 회귀 검증(Regression Testing)
> : 소프트웨어의 코드를 수정하거나 새로운 기능을 추가한 후,
>   "기존에 잘 작동하던 기능에 문제가 생기지 않았는가?"를 확인하는 테스트 과정
...
* `moving-arrows`는 원본의 애니메이션 대신 점선(dashed) 화살표로 근사 구현
  (2차에서 애니메이션 재현 여부 검토=> GUI 새 버전 README에 관련하여 서술)
* HTML/CSS 마크업 구조는 원본과 동일하지 않음
* 오류 메시지 문구는 원본과 다를 수 있음 
  (기능적으로는 동일한 4가지 오류 유형을 검증: 
   잘못된 섹션명 / 미정의 Class / 미정의 Relation / 미정의 Node)

> 이러한 차이는 2차 구현에서 원본과 나란히 비교하며 좁혀 나가고자 했으나,
> 원본을 완벽히 구현하기보다는 온톨로지를 제대로 구축하는 게 낫다고 생각하여 
> 원본과 추가 비교 분석은 거의 하지 않았다.

---

## 9. 라이선스 및 출처

> 재구현 코드는 [MIT License](LICENSE)를 따릅니다.
> 원본 `makeGraph2022.exe`의 저작권 및 출처는 [NOTICE.md](NOTICE.md)를 참고하세요.
